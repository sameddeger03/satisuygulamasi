...
import { ipcMain, net, BrowserWindow, app, shell, session } from "electron";
...
function logger(...mesajlar) {
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    console.log(clc.blue(moment(/* @__PURE__ */ new Date()).format("DD-MM-YYYY HH:mm:ss") + ": "), ...cMesajlar);
  }
}
class ayarlarVT {
  constructor(db) {
...
class kategorilerVT {
  ...
  async bul(id, isimMi = false, silinmisObje = false) {
    try {
      const istek = this.db.select("*").from("kategoriler");
      if (isimMi)
        istek.where("isim", id);
      else
        istek.where("id", id);
  ...
  async sil(id) {
    try {
      const kategori = await this.bul(id);
      kategori.sil = true;
...
function sqlTarih(tarih = /* @__PURE__ */ new Date()) {
  const tamTarih = sqlTamTarih(tarih);
  let [trh, st] = tamTarih.split(" ");
  let [HH, MM, SS] = st.split(":");
  return `${trh} ${HH}:${MM}`;
}
function sqlTamTarih(tarih = /* @__PURE__ */ new Date()) {
...
function sqlTarihSaatsiz(tarih = /* @__PURE__ */ new Date()) {
  return sqlTarih(tarih).split(" ")[0];
}
function sqlSaat(tarih = /* @__PURE__ */ new Date()) {
  return sqlTarih(tarih).split(" ")[1];
}
class modalpencere {
  constructor() {
    __publicField(this, "modalList", {});
    setInterval(() => {
...
class urunlerVT {
  constructor(db) {
    __publicField(this, "db");
    __publicField(this, "toplamUrun", 0);
    this.db = db;
    this.toplamKaydet();
  }
  toplamKaydet() {
    this.db("urunler").count({ count: "*" }).join("kategoriler", "urunler.kategori", "=", "kategoriler.id").select("urunler.*", "kategoriler.isim as kategori_isim").where("urunler.sil", 0).andWhere("kategoriler.sil", 0).first().then((toplam) => {
      this.toplamUrun = toplam.count;
    });
  }
  async liste(sayfa, filtreler, sira) {
    ...
    try {
      const offset2 = (sayfa - 1) * 25;
      let filtreliToplam = this.toplamUrun;
      let urunler2 = await this.db("urunler").join("kategoriler", "urunler.kategori", "=", "kategoriler.id").select("urunler.*", "kategoriler.isim as kategori_isim").where("urunler.sil", 0).andWhere("kategoriler.sil", 0);
      urunler2.sort((a, b) => {
      ...
      for (let i = 0; i < urunler2.length; i++) {
        const urun = urunler2[i];
        if (filtreler.length) {
          let sil = false;
      ...
      if (filtreler.length) {
        filtreliToplam -= urunler2.length;
      }
      urunler2 = urunler2.slice(offset2, offset2 + 25);
      let topSayfa = 1;
  ...
  async favs() {
    try {
      const urunler2 = await this.db.select("*").from("urunler").where("fav", true).andWhere("sil", 0);
      return urunler2;
    } catch (error) {
      logger(error);
      return [];
  ...
  async bul(id, isBarkod = true, force = false, duzelt = true) {
    try {
      ...
      if (isBarkod) {
        ...
        const urun = await sql.first();
        if (urun) {
          if (duzelt) {
            urun.adet = 1;
            if (miktar != null) {
              if (urun.birim == "kilo") {
                const kgValue = Math.floor(Number(miktar) / 10) / 1e3;
  ...
  async sil(id) {
    let sonuc = false;
    try {
      const urun = await this.bul(id, false);
...
class odemelerVT {
  ...
  async ekle(request) {
    try {
      const kontrol2 = await this.db.select("*").from("ayarlar").where("degisken", "kurusDuzelt").first();
      if (kontrol2 && (kontrol2.deger == "1" || kontrol2.deger == true)) {
        const yeniIcerik = [];
  ...
  async iade(odemeid, iadeurunler, neden) {
    try {
      const odeme = await this.bul(odemeid);
      if (typeof odeme == "boolean") return odeme;
      const json = JSON.parse(odeme.icerik);
      json.forEach((urun) => {
        iadeurunler.forEach((iadeurun) => {
      ...
      const iade = { odemeid: parseInt(odeme.id.toString()), icerik: JSON.stringify(iadeurunler), neden: neden.toString(), tarih: sqlTarih(), sil: false };
...
class iadelerVT {
  ...
  async iptal(id) {
    try {
      const iadeOdeme = await this.bul(id);
      if (typeof iadeOdeme == "boolean") return iadeOdeme;
      const odeme = await this.db("odemeler").where("id", iadeOdeme.odemeid).first();
      if (typeof odeme == "boolean") return odeme;
      const jsonIadeler = JSON.parse(iadeOdeme.icerik);
      const jsonUrunler = JSON.parse(odeme.icerik);
      jsonIadeler.forEach((iadeurun) => {
        jsonUrunler.forEach((urun) => {
          if (urun.id == iadeurun.id) {
            urun.iade -= iadeurun.adet;
            urun.adet += iadeurun.adet;
          }
...
class planlamaVT {
  ...
  async liste(tarih = /* @__PURE__ */ new Date(), ozel = "") {
    try {
      ...
      if (ozel != "") {
        switch (ozel) {
          default:
            return [];
          case "alarm":
            return await this.db.select("*").from("alarmlar").whereRaw("STRFTIME('%Y-%m-%d', tarih) = ?", [targetDate]).where("sil", 0);
          case "plan":
            return await this.db.select("*").from("planlar").whereRaw("STRFTIME('%Y-%m-%d', tarih) = ?", [targetDate]).where("sil", 0);
        }
      } else {
        const alarmlar = await this.db.select("*").from("alarmlar").whereRaw("STRFTIME('%Y-%m-%d', tarih) = ?", [targetDate]).where("sil", 0);
        const planlar2 = await this.db.select("*").from("planlar").whereRaw("STRFTIME('%Y-%m-%d', tarih) = ?", [targetDate]).where("sil", 0);
        return { planlar: planlar2, alarmlar };
      }
    } catch (error) {
      logger(error);
  ...
  isHatirlatma(request) {
    return request.baslik !== void 0;
  }
  isPlan(request) {
    return request.tip !== void 0;
  }
  async bul(id, ne) {
    try {
  ...
  async sil(id, ne) {
    try {
      if (ne == "alarm") {
        const alarm = await this.bul(id, "alarm");
        if (typeof alarm == "boolean") return alarm;
        alarm.sil = true;
        await this.guncelle(alarm);
        return true;
      } else if (ne == "plan") {
        const plan = await this.bul(id, "plan");
...
class ckVT {
  ...
  async liste() {
    ...
    try {
      ls.urunler = await this.db("urunler").join("kategoriler", "urunler.kategori", "=", "kategoriler.id").select("urunler.*", "kategoriler.isim as kategori_isim").where("urunler.sil", 1);
      ...
      ls.odemeler.forEach((e) => {
        const icerik = [];
        JSON.parse(e.icerik).forEach((i) => {
          icerik.push(i.isim);
        });
...
class vt {
  constructor(dbPath, yedekPath) {
    __publicField(this, "dbPath");
    __publicField(this, "yedekPath");
    __publicField(this, "db");
    __publicField(this, "kategoriler");
    __publicField(this, "urunler");
    __publicField(this, "odemeler");
    __publicField(this, "iadeler");
    __publicField(this, "planlama");
    __publicField(this, "ayarlar");
    __publicField(this, "notdefteri");
    __publicField(this, "paketler");
    ...
    try {
      ...
      if (!await db.schema.hasTable("urunler")) {
        await db.schema.createTable("urunler", (table) => {
          ...
          table.boolean("fav").defaultTo(false);
          table.string("imaj").defaultTo("");
          table.boolean("sil").defaultTo(false);
        });
      }
        ...
        for (const tablo in sonradanCikarildi) {
          ...
          for (const ne of sutunlar) {
            ...
            if (sutunVar) {
              ...
              const newColumns = columns.filter((col) => col.name !== ne).map((col) => ({
                name: col.name,
                type: col.type,
                notnull: col.notnull,
    ...
    try {
      ...
      for (let i = 3; i < list.length; i++) {
        if (list[i]) {
          ...
          reallist.push({ name: processName, pid });
        }
      }
...
class govde {
  constructor(pencere2, uygulama2) {
    __publicField(this, "islemci", (mainWindow, uygulama2) => {
      ipcMain.on("yenile", () => {
      });
      ipcMain.on("onegetir", () => {
    ...
    this.islemci(pencere2, uygulama2);
  }
}
...
class internet {
  constructor() {
    __publicField(this, "zamanlayici");
    sonPingSonuc = false;
    sonPingSonuc2 = false;
  }
  zamanla() {
...
class urunler {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("urun-liste", async (event, sayfa, filitre, sira) => {
        if (!sayfa || sayfa < 1) sayfa = 1;
        this.vt.urunler.liste(sayfa, filitre, sira).then((liste) => {
      ...
      ipcMain.on("urun-favs", async (event) => {
        this.vt.urunler.favs().then((liste) => {
      ...
      ipcMain.on("urun-fav-degis", async (event, id) => {
        console.log(id);
      ...
      ipcMain.on("urun-ekle", async (event, request) => {
        this.vt.urunler.bul(request.barkod, true, true).then((bul) => {
          if (bul && !bul.sil) {
            event.reply("urun-ekle-response", {
              durum: false,
              mesaj: "Ürün zaten kayıtlı."
            });
          } else this.vt.urunler.ekle(request).then((sonuc) => {
            if (sonuc) {
              event.reply("urun-ekle-response", {
                durum: true,
      ...
      ipcMain.on("urun-duzenle", async (event, request) => {
        this.vt.urunler.bul(request.id, false).then((urun) => {
          this.vt.urunler.guncelle(request).then((sonuc) => {
            if (sonuc) {
      ...
      ipcMain.on("urun-sil", async (event, id) => {
        this.vt.urunler.sil(id).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("urun-bul", async (event, barkod) => {
        this.vt.urunler.bul(barkod).then((urun) => {
          if (urun)
...
class kategoriler {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("kategori-liste", async (event) => {
        this.vt.kategoriler.liste().then((liste) => {
      ...
      ipcMain.on("kategori-ekle", (event, kategori) => {
        this.vt.kategoriler.bul(kategori.isim, true).then((bul) => {
          if (bul && !bul.sil) {
            ...
            this.vt.kategoriler.ekle(kategori).then((sonuc) => {
              if (sonuc)
                event.reply("kategori-ekle-response", {
                  durum: true,
      ...
      ipcMain.on("kategori-duzenle", (event, kategori) => {
        this.vt.kategoriler.guncelle(kategori).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("kategori-sil", (event, id) => {
        this.vt.kategoriler.urunVarmi(id).then((durum) => {
          if (durum) {
            ...
            this.vt.kategoriler.sil(id).then((sonuc) => {
              if (sonuc)
                event.reply("kategori-sil-response", {
                  durum: true,
...
class odemeler {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("odeme-liste", async (event, tarih) => {
        this.vt.odemeler.liste(tarih).then((liste) => {
      ...
      ipcMain.on("rapor-liste", async (event) => {
        this.vt.odemeler.liste().then((odemeler2) => {
          this.vt.iadeler.liste().then((iadeler2) => {
            event.reply("rapor-liste-response", {
              durum: true,
              mesaj: "",
              veri: { odemeler: odemeler2, iadeler: iadeler2 }
            });
          });
        });
      });
      ipcMain.on("odeme-ekle", async (event, odeme) => {
        this.vt.odemeler.ekle(odeme).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("odeme-sil", async (event, id) => {
        this.vt.odemeler.sil(id).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("odeme-iade", async (event, id, iadeList, neden) => {
        this.vt.odemeler.iade(id, iadeList, neden).then((sonuc) => {
          if (sonuc)
...
class iadeler {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("iade-liste", async (event) => {
        this.vt.iadeler.liste().then((liste) => {
      ...
      ipcMain.on("iade-sil", async (event, id) => {
        this.vt.iadeler.sil(id).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("iade-iptal", async (event, id) => {
        this.vt.iadeler.iptal(id).then((sonuc) => {
          if (sonuc)
            event.reply("odeme-iptal-response", {
              durum: true,
      ...
      ipcMain.on("iade-neden", async (event, id) => {
        this.vt.iadeler.bul(id).then((iade) => {
          if (iade)
...
class ayar {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("sirket-isim", (event) => {
        this.vt.ayarlar.get("sirket").then(
          (sonuc) => event.reply("sirket-isim-response", sonuc)
        );
      });
      ipcMain.on("ayar-kaydet", async (event, ayar2) => {
        try {
          await this.vt.ayarlar.set(ayar2.degisken, ayar2.deger.toString());
          event.reply("ayar-kaydet-response", true);
        } catch (error) {
          console.error("Ayar kaydederken hata oluştu:", error, ayar2);
          event.reply("ayar-kaydet-response", false);
        }
      });
      ipcMain.on("ayarlari-kaydet", async (event, ayarlar) => {
        try {
          for (const ayar2 of ayarlar) {
            await this.vt.ayarlar.set(ayar2.degisken, ayar2.deger);
          }
...
class gunSonuRaporu {
  ...
  async raporla(tarih) {
    let rapor = this.bosrapor;
    const satislar = await this.vt.odemeler.liste(tarih);
    const iadeler2 = await this.vt.iadeler.liste(tarih);
    const musteriSayisi = satislar.length;
    const iadeSayisi = iadeler2.length;
    let urunSay = { pos: 0, nakit: 0, iade: 0 };
    let toplamSatis;
    let nakit = 0;
    let kredi = 0;
    let nakitKazanc = 0;
    ...
    satislar.forEach((satis) => {
      ...
      const urunler2 = JSON.parse(satis.icerik);
      urunler2.forEach((urun) => {
        if (satis.odeme) {
          if (urun.birim == "adet")
    ...
    iadeler2.forEach((iade) => {
      const urunler2 = JSON.parse(iade.icerik);
      urunSay.iade++;
      const temizListe = urunler2.reduce((acc, curr) => {
    ...
    if (saatler.length > 1) {
      const timeStamps = saatler.map((time) => {
        ...
        const date = new Date(1970, 0, 1, hours2, minutes2);
        return date.getTime();
      });
...
function alarmlariilet(vt2, tarih) {
  vt2.planlama.liste(tarih, "alarm").then((liste) => {
...
class planlar {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("planlamalar", async (event, tarih) => {
        this.vt.planlama.liste(tarih).then((liste) => {
      ...
      ipcMain.on("alarm-ekle", async (event, request) => {
        this.vt.planlama.ekle(request).then((sonuc) => {
          if (sonuc)
          ...
          alarmlariilet(this.vt, /* @__PURE__ */ new Date());
        });
      });
      ipcMain.on("alarm-sil", async (event, id) => {
        this.vt.planlama.sil(id, "alarm").then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("alarmlar", async (event, tarih = null) => {
        alarmlariilet(this.vt, tarih);
      });
      ipcMain.on("alarmUyar", async (event, id, eylem) => {
        this.vt.planlama.alarmUyar(id, eylem).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("plan-ekle", async (event, request) => {
        this.vt.planlama.ekle(request).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("plan-sil", async (event, id) => {
        this.vt.planlama.sil(id, "plan").then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("plan-yapildi", async (event, id, eylem) => {
...
class notlar {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("not-liste", async (event) => {
        this.vt.notdefteri.liste().then((liste) => {
      ...
      ipcMain.on("not-ekle", async (event, request) => {
        this.vt.notdefteri.ekle(request).then((id) => {
          if (id > -1)
      ...
      ipcMain.on("not-sil", async (event, id) => {
        this.vt.notdefteri.sil(id).then((sonuc) => {
          if (sonuc)
      ...
      ipcMain.on("not-guncelle", async (event, request) => {
        request.tarih = sqlTarih();
        this.vt.notdefteri.guncelle(request).then((sonuc) => {
        });
      });
    });
    ...
    Object.keys(db).forEach(function forEachMimeType(type) {
      ...
      for (var i = 0; i < exts.length; i++) {
        ...
        if (types[extension2]) {
          ...
          if (types[extension2] !== "application/octet-stream" && (from2 > to2 || from2 === to2 && types[extension2].substr(0, 12) === "application/")) {
            continue;
          }
        }
...
async function tarihGetir(token2) {
  try {
    const response = await axios.get(anadom + "/satisuygulamasi/yedek/tarih/" + machineIdSync() + "/" + token2);
    return response.data;
  } catch (error) {
    return { durum: false, mesaj: error.message, token: "" };
  }
}
async function yukle(dosyaYolu, token2) {
  try {
    var formData = new FormData$1();
    formData.append("db", fs$1.createReadStream(dosyaYolu));
    const response = await axios.post(anadom + "/satisuygulamasi/yedek/yukle/" + machineIdSync() + "/" + token2, formData, {
      headers: {
...
function enYeniDosyayiBul(dizinYolu) {
  const dosyalar = fs$1.readdirSync(dizinYolu);
  let enYeniDosya = null;
  let enYeniZaman = 0;
  dosyalar.forEach((dosya) => {
    const dosyaYolu = path$1.join(dizinYolu, dosya);
    const dosyaBilgisi = fs$1.statSync(dosyaYolu);
...
async function indir(dosyaYolu, token2) {
  try {
    const response = await axios.get(anadom + "/satisuygulamasi/yedek/indir/" + machineIdSync() + "/" + token2, {
      responseType: "arraybuffer"
    });
    const fileData = Buffer.from(response.data, "binary");
    fs$1.writeFileSync(dosyaYolu, fileData);
    return { durum: true, mesaj: "Yedek dosyası indirildi.", token: token2 };
  } catch (error) {
    if (error.response.status == 404)
      return { durum: false, mesaj: "Yedek dosyası bulunamadı.", token: token2 };
    if (error.response.status == 403)
      return { durum: false, mesaj: "Token geçersiz.", token: null };
    logger(error);
    return { durum: false, mesaj: error.message, token: token2 };
  }
}
let yedeklemeZamanlayiciLokal = null;
let yedeklemeZamanlayiciRemote = null;
function yedekDosyalariSil(yedekYol) {
  const dosyalar = fs$1.readdirSync(yedekYol).map((dosya) => {
    const dosyaYolu = path$1.join(yedekYol, dosya);
    const istatistik = fs$1.statSync(dosyaYolu);
    return {
      dosyaYolu,
      dosyaAdi: dosya,
      zamanDamgasi: istatistik.mtimeMs
    };
  }).filter((dosya) => path$1.extname(dosya.dosyaAdi) === ".sqlite").sort((a, b) => b.zamanDamgasi - a.zamanDamgasi);
  const simdi = Date.now();
  const ucGunMilisaniye = 3 * 24 * 60 * 60 * 1e3;
  dosyalar.forEach((dosya, index) => {
    if (index >= 10 || simdi - dosya.zamanDamgasi > ucGunMilisaniye) {
      fs$1.unlinkSync(dosya.dosyaYolu);
      logger(`${dosya.dosyaAdi} silindi.`);
    }
  });
}
function dosyaHashOlustur(dosyaYolu) {
  return new Promise((resolve, reject) => {
    const hash = crypto.createHash("sha256");
    const stream = fs$1.createReadStream(dosyaYolu);
    stream.on("data", (data) => {
      hash.update(data);
    });
    stream.on("end", () => {
...
async function dosyalariKarsilastir(dosyaYolu1, dosyaYolu2) {
...
async function paketyukle(vt2, paket) {
  try {
      ...
      switch (response.data.tablo) {
        case "urunler":
          ...
          for (let urun of response.data.icerik) {
            ...
            const bul = await vt2.urunler.bul(urun.barkod, true, true);
            if (bul) {
              if (degisim == "*") {
                await vt2.urunler.guncelle(urun);
              } else {
                for (let i = 0; i < degisim.length; i++)
                  bul[degisim[i]] = urun[degisim[i]];
                bul.sil = 0;
                delete bul.adet;
                await vt2.urunler.guncelle(bul);
              }
            } else {
              await vt2.urunler.ekle(urun);
            }
          }
      }
...
class yedek {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("paket-kontrol", async (event) => {
        dm().then(async () => {
          await paketKontrol();
          for (const paket of paketler) {
            ...
            if (mevcutPaket) {
              ...
              await this.vt.paketler.ekle({ id: null, paket });
            }
          }
      ...
      ipcMain.on("oto-yedekleme-lokal", (event, durum) => {
        ...
        if (durum) {
          yedeklemeZamanlayiciLokal = setInterval(async () => {
            if (await dosyalariKarsilastir(enYeniDosyayiBul(this.vt.yedekPath), this.vt.dbPath) == false) {
              const yedekdosyasi = this.vt.yedekPath + "\\su.yedek." + (/* @__PURE__ */ new Date()).getTime() + ".sqlite";
              fs$1.copyFileSync(this.vt.dbPath, yedekdosyasi);
              fs$1.utimesSync(yedekdosyasi, /* @__PURE__ */ new Date(), /* @__PURE__ */ new Date());
            }
            yedekDosyalariSil(this.vt.yedekPath);
          }, 1e3 * 60 * 60);
        }
      });
      ipcMain.on("oto-yedekleme-remote", (event, bilgi) => {
      ...
      ipcMain.handle("macid", async (event) => {
        return machineIdSync();
      });
      ipcMain.handle("giris-yap", async (event, veriler) => {
        veriler.giris = 1;
        try {
      ...
      ipcMain.handle("yedek-ver", async (event, token2, local = false) => {
        if (local) {
          logger("LOCAL YEDEK");
          try {
            const yedekdosyasi = this.vt.yedekPath + "\\su.yedek." + (/* @__PURE__ */ new Date()).getTime() + ".sqlite";
          ...
          try {
            return await yukle(this.vt.dbPath, token2);
          } catch (error) {
            logger(error);
            return { durum: false, mesaj: error.message, token: "" };
          }
      });
      ipcMain.handle("yedek-al", async (event, token2, local = false) => {
        if (local)
          try {
            fs$1.copyFileSync(enYeniDosyayiBul(this.vt.yedekPath), this.vt.dbPath);
          ...
          try {
            return await indir(this.vt.dbPath, token2);
          } catch (error) {
            logger(error);
            return { durum: false, mesaj: error.message, token: "" };
          }
      });
      ipcMain.handle("son-yedek-tarihi", async (event, token2, local = false) => {
        if (local)
          try {
            const yedek_ = enYeniDosyayiBul(this.vt.yedekPath);
            if (yedek_ == null) return { durum: false, mesaj: null, token: "local" };
            const stats = fs$1.statSync(yedek_);
          ...
          try {
            return await tarihGetir(token2);
          } catch (error) {
            logger(error);
...
class copkutusu {
  constructor(veritabani2) {
    __publicField(this, "vt");
    __publicField(this, "islemci", () => {
      ipcMain.on("ck-liste", async (event) => {
        this.vt.ck.liste().then((liste) => {
      ...
      ipcMain.on("ck-kurtar", async (event, gelen) => {
        const { id, tablo } = gelen;
        this.vt.ck.kurtar(id, tablo).then((sonuc) => {
      ...
      ipcMain.on("ck-sil", async (event, gelen) => {
        const { id, tablo } = gelen;
        this.vt.ck.kurtar(id, tablo).then((sonuc) => {
          if (sonuc) {
...
class Dinleme {
  basla(uygulama2, pencere2, veritabani2) {
    new govde(pencere2, uygulama2);
    new kontrol(veritabani2);
    new ayar(veritabani2);
    new urunler(veritabani2);
    new kategoriler(veritabani2);
    new odemeler(veritabani2);
    new iadeler(veritabani2);
    new gunSonuRaporu(veritabani2);
    new planlar(veritabani2);
    new notlar(veritabani2);
    new yedek(veritabani2);
    new copkutusu(veritabani2);
  }
}
const { autoUpdater } = pkg$1;
const uygulama = app;
let anaPencere = null;
let guncellemePencere = null;
...
path$1.join(RENDERER_DIST, "bos.html");
const indexHtml = path$1.join(RENDERER_DIST, "index.html");
const lockHtml = path$1.join(RENDERER_DIST, "lock.html");
const guncellemeHtml = path$1.join(RENDERER_DIST, "guncelleme.html");
let veritabani = null;
let internetKontrol;
let appYolu = "";
let yedekYolu = "";
...
async function pencere() {
  ...
  if (fs$1.existsSync(appYolu) === false) fs$1.mkdirSync(appYolu);
  yedekYolu = appYolu + "\\Yedekler";
  if (fs$1.existsSync(yedekYolu) === false) fs$1.mkdirSync(yedekYolu);
  raporYolu = appYolu + "\\Raporlar";
  if (fs$1.existsSync(raporYolu) === false) fs$1.mkdirSync(raporYolu);
  veriTabaniYolu = path$1.join(uygulama.getPath("userData"), "satisuygulamasi.db");
  yazdirStilYolu = path$1.join(process.env.VITE_PUBLIC, "yazdir.css");
  if (fs$1.existsSync(yazdirStilYolu) === false) fs$1.writeFileSync(yazdirStilYolu, "");
  yazdirStil = fs$1.readFileSync(yazdirStilYolu).toString();
...
ipcMain.handle("versiyon", async (event) => {
...
ipcMain.on("sifre", async (event, sifre) => {
...
ipcMain.on("pdf", async (event, baslik, html) => {
  ...
  printWin.webContents.printToPDF({}).then((data) => {
    const pdfPath = path$1.join(raporYolu, baslik + ".pdf");
    fs$1.writeFile(pdfPath, data, (error) => {
